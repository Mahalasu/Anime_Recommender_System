...
# from recall_service.recall.util import bucketize
...
def anime_rank(context):
    ...
    # if bucket == 1:
    #     rank_result = mlp_rank(context, recall_res)
...
def mlp_rank(user_id, recall_res):
    ...
    item_num_feature_list = [
        dataset.get_item_numeric_features(item_id) for item_id in recall_res
    ]
    item_cat_feature_list = [
        dataset.get_item_categorical_features(item_id) for item_id in recall_res
    ]

    # merge features
    model_inputs = __build_features(
        item_cat_feature_list,
        user_cat_features,
        item_num_feature_list,
        user_num_features,
...
def __build_features(item_cats, user_cats, item_nums, user_nums):
    ...
    # x2: user cat
    x2 = [
        np.nan_to_num(np.array(user_cats['user_liked_genres_multihot']))
        for _ in item_cats
    ]
